...
export function DashboardPage(): JSX.Element {
  ...
  const refresh = useCallback(async () => {
    if (!current) return;
    try {
      const [i, p, pr, r, plr, rp, m, rep] = await Promise.all([
  ...
  return (
    <Page>
      ...
      <UsageSection runs={runs.filter((r) => r.repo === null || wsRepoNames.has(r.repo))} />
      ...
      <div className="mt-6 grid gap-5 lg:grid-cols-2">
        ...
        <section aria-labelledby="live-activity">
          ...
          <ActivityFeed
            runs={runs.filter((r) => r.repo === null || wsRepoNames.has(r.repo))}
            pipelineRuns={pipelineRuns}
            reports={reports.filter((r) => r.repo === null || wsRepoNames.has(r.repo))}
          />
        </section>
      </div>
    </Page>
