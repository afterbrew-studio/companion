...
import type { NotificationRecord, NotificationScope } from '@companion/contract';
...
export function useNotifications(): UseNotifications {
  ...
  const refresh = useCallback(async () => {
    try {
      const { notifications } = await api.listNotifications(workspaceArg);
  ...
  const markAllRead = useCallback((): void => {
    void api.markAllNotificationsRead(workspaceArg).catch(() => undefined);
  }, [workspaceArg]);
