...
import type { WorkspaceMember } from '@companion/contract';
...
export function useWorkspaceMembers(workspaceId: string): {
  ...
  const add = async (username: string): Promise<void> => {
    setError(null);
    const { members } = await api.addWorkspaceMember(workspaceId, username);
    setMembers(members);
  };
  ...
  const remove = async (name: string): Promise<void> => {
    setError(null);
    try {
      const { members } = await api.removeWorkspaceMember(workspaceId, name);
      setMembers(members);
    } catch (err) {
      setError(String(err));
    }
