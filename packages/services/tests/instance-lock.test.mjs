...
function withHome(fn) {
  ...
  try {
    return fn(join(home, 'instance.lock'));
  } finally {
...
test('acquiring writes a lock naming this process, and releasing removes it', () => {
  withHome((file) => {
...
test('a second daemon on the same home refuses, naming the holder', () => {
  withHome((file) => {
    ...
    assert.throws(() => new InstanceLock().acquire(), (err) => {
...
test('a dead process on this host is taken over immediately, not after a timeout', () => {
  withHome((file) => {
...
test('another host is judged by heartbeat only, since its pid means nothing here', () => {
  withHome((file) => {
    ...
    assert.throws(() => new InstanceLock().acquire(), /already using/);
...
test('releasing does not delete a lock another daemon has since taken', () => {
  withHome((file) => {
    const lock = new InstanceLock();
    lock.acquire();
...
test('an unreadable lock is treated as absent rather than fatal', () => {
  withHome((file) => {
