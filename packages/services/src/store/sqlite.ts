...
export class Database {
  ...
  /**
   * `readOnly` is spelled the way `node:sqlite` spells it, and that matters more
   * than it looks: better-sqlite3 called it `readonly`, and passing that spelling
   * here is accepted and silently ignored, leaving a writable handle. Measured.
   * A backup verified through a handle that turned out to be writable is the
   * failure this naming prevents.
   */
  constructor(path: string, opts: { readonly readOnly?: boolean } = {}) {
