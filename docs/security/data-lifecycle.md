...
| --- | --- | --- | --- |
| Users, roles, browser sessions, API-token metadata | `companion.db` under `COMPANION_HOME` | Browser sessions expire after 7 days; account disable, role/password change, and deletion revoke relevant sessions/tokens | Administrators manage users/tokens. There is no per-browser session inventory yet; this is a recorded residual control. |
