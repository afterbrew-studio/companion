import { Eyebrow, ListCard, MetaSignal, RowsSkeleton } from '@moxxy/companion-sdk/ui';
 ...
 * the scan is the strengths, the reading is whichever rows earn it.
 */
export function SlopSignals({ signals }: { signals: ReadonlyArray<SlopSignal> }): JSX.Element {
  return (
    <div>
      <Eyebrow>Signals ({signals.length})</Eyebrow>
      <ListCard className="mt-1.5" ariaLabel="Signals, strongest first">
        {rankSignals(signals).map((signal, i) => (
          <details key={`${signal.ruleId}-${i}`} className="group">
            <summary className="flex cursor-pointer list-none items-center gap-2 px-4 py-2.5 transition-colors select-none hover:bg-zinc-50 dark:hover:bg-zinc-900">
            ...
            <p className="px-4 pb-3 text-[13px] wrap-anywhere sm:pl-26">{signal.observation}</p>
          </details>
        ))}
      </ListCard>
    </div>
